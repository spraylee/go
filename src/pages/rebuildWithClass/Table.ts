...
export class Table {
  ...
  private lineByFourDirectionAboutLastMove() {
    const last = this.getLastMove()
    ...
    return offset.map(line => line.map(([i, j]) => this.getCell(last.x + i, last.y + j)))
  }
}
